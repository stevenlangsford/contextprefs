...
mystim <- wedellTextExample_twomatrixformat()
...
write.csv(simexp.df,file=paste0("simexp_textexample.csv"),row.names=FALSE)

endtime=Sys.time();


simexp.df$trialtype <- as.character(map(simexp.df$trialid,function(anid){
    if(anid==1)return( "R decoy: (.5,$20)|(.4, $25)|(.5,$18)")
    if(anid==2)return( "R non-decoy (.5,$20)|(.4, $25)|(.1,$1)")
    if(anid==3)return( "F decoy (.67,$15)|(.5, $20)|(.5,$18)")
    if(anid==4)return( "F non-decoy (.67,$15)|(.5, $20)|(.1,$1)")
}))

prefs.plot <- ggplot(simexp.df,aes(x=choice))+geom_bar()+facet_wrap(~trialtype)+theme_bw()

ggsave(prefs.plot,file="prefs_plot.png")
