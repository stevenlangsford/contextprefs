...
data{
  ...
  //agent setup passed because agents may differ
//There are more params than these in the full model: assuming alpha=1 and and U() is identity (for now)
  real calc_sd;
  //  real p_err = .01; //set in get_ordtheta function.
  real tolerance_prob; //tolerance vals from pg 374 LHS point 3.
  real tolerance_payoff;
}

parameters{
  //this model is aware of its own calc_sd etc values, just uncertain about the option attribute values.
