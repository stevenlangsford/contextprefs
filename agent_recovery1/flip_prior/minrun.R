...
simexp.df <- read.csv(file="simexp.csv") #super-interesting stim exp with some choices attached.

starttime=Sys.time()
recovery.fit <- as.data.frame(webppl(program_file="recover.ppl",data=simexp.df,data_var="expdf")) #these.trials[1:i,]
endtime=Sys.time()
