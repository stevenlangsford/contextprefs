...
starttime=Sys.time()
recovery.fit <- as.data.frame(webppl(program_file="asyougo_recovercalcsd.ppl",data=use.these.trials,data_var="expdf"))
endtime=Sys.time()
...
save.image("demorecovery1.RData");
...
## bob <- data.frame(rows=c(2,4,8,12,14,16),time=c(8,7.7,12.5,29,4.05*60,420))
## ggplot(bob,aes(x=2^rows,y=time))+geom_point()+geom_line()+theme_bw()
