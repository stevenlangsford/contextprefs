...
getB <- function(){
    return(runif(1,0,10)); #return(runif(1,0,10))#Goodness of B is expressed as some scaling factor of goodness of A. Both features are assumed to be positive for now?
}
getcalcsd <- function(){
                                        #return(.5); ##? reasonable?
    return(runif(1,.5,5))
...
ppnt_Bweight <- rep(2,hm_ppnts);#runif(hm_ppnts,0,5)
