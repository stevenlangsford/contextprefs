...
ggsave(ggplot(est.df,aes(x=est))+geom_histogram()+theme_bw()+facet_wrap(~ppntid)+geom_vline(data=simtruth.df,aes(xintercept=Bweight)),file="fithist.png")

ggsave(ggplot(est.summary.df,aes(x=simtruth,y=meanB))+geom_point()+theme_bw(),file="fitpoints.png")
