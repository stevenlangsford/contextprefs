...
(ggplot(est.summary.df,aes(x=simtruth,y=meanB))+geom_point()+theme_bw()+ylim(c(0,1))+
 ggplot(simexp.df%>%group_by(trialid,ppntid)%>%summarize(prop1=sum(choice==1)/n()),aes(x=trialid,y=prop1,color=ppntid,group=ppntid))+geom_point()+geom_line()+theme_bw()+facet_wrap(~ppntid)+guides(color=FALSE))#,file="twooption_failure.png",width=15)
