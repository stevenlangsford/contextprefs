...
for(afile in list.files("logs/")){
    ...
    incoming <- as.data.frame(fromJSON(paste0("logs/",afile)))
    log.df <- rbind(log.df,incoming)
}

 log.df$ppntid <- as.factor(log.df$ppntid)


optionsummary.df <- log.df%>%group_by(simA1,simA2,simB1,simB2)%>%summarize(
                                                   myA1=mean(myA1),
                                                   myA2=mean(myA2),
                                                   myB1=mean(myB1),
                                                   myB2=mean(myB2)
                                                   )%>%ungroup()
checkoptionests.df <- bind_cols(optionsummary.df%>%select(contains("sim"))%>%gather(whichoption,value,1:4),optionsummary.df%>%select(contains("my"))%>%gather(whichoption,value,1:4)) ##edited from 1:6
...
x11(); print(Brecovery.plot+checkoptionests.plot,file="midrun_Bweight_optionests.png",width=15)

print("mychoice:simchoice agreement")
print(with(log.df,sum(mychoice==simchoice)/nrow(log.df)))
print("simchoice:oracle agreement")
...
print("mychoice:oracle agreement")
...
x11(); print(bhist.plot,file="bhists.png")
