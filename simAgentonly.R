...
library(ggplot2)
rm(list=ls())
if(any(list.files()=="threefits.RData")){
    load("threefits.RData")#skip fitting.
}else{
...
timer_sanscalc <- system.time(
    fit_ordonly<- stan(file="ordobs_only.stan",data=datalist,warmup=stan_warmup,iter=stan_iter,chains=4, control = list(max_treedepth = stan_treedepth))
)
timer_sansord <- system.time(
    fit_calconly<- stan(file="calcobs_only.stan",data=datalist,warmup=stan_warmup,iter=stan_iter,chains=4,control = list(max_treedepth = stan_treedepth))
)
timer_full <- system.time(
    fit_full<- stan(file="howes16.stan",data=datalist,warmup=stan_warmup,iter=stan_iter,chains=4,control = list(max_treedepth = stan_treedepth))
)

save.image(file="threefits.RData");
View("done");
