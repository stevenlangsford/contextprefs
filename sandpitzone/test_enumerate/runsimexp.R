...
timer1 <- system.time({
simexp.df$choice <- webppl(program_file="ABchoices_original.ppl",data=simexp.df,data_var="expdf",packages=c("webppl-json"))
});

print("staring enumerate");
timer2 <- system.time({
simexp.df$choice <- webppl(program_file="ABchoices_enumerate.ppl",data=simexp.df,data_var="expdf",packages=c("webppl-json"))
})
