...
#ggsave(
ggplot(mysummary.df)+
    geom_line(aes(x=Bweight,y=Bweight,color="identity"),alpha=.5)+geom_point(aes(x=Bweight,y=Bweight,color="identity"),alpha=.5,size=2)+
    geom_point(aes(x=Bweight,y=est,color="recovered"))+
    ylim(c(0,1))+xlab("simtruth")+ylab("recovered")+
theme_bw()#,
#file="when_value_is_visible.png"
#)
