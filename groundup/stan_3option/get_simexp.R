...
hm_ppnts <- 20;
hm_trials <- 50;
trialreps <- 1; #if choices are noisy, consider repeating stim.
stimrange <- 1; #stim attributes in 0:stimrange
...
stim.df <- data.frame(
    ...
    attributeA_option3=runif(hm_trials,0,stimrange),
    attributeB_option1=runif(hm_trials,0,stimrange),
    attributeB_option2=runif(hm_trials,0,stimrange),
    attributeB_option3=runif(hm_trials,0,stimrange),
    trialid=1:hm_trials
)
...
getoraclechoice <- function(options){
        which(options==max(options))[base::sample(length(which(options==max(options))),1)] #sample bs is in case of a tie, should be uber-rare.
}
...
for(i in 1:nrow(simexp.df)){
    ...
    simexp.df[i,"value3"]=getvalue(simexp.df[i,"attributeA_option3"],simexp.df[i,"attributeB_option3"],simexp.df[i,"ppntid"])
    simexp.df[i,"oraclechoice"]=getoraclechoice(c(simexp.df[i,"value1"],simexp.df[i,"value2"],simexp.df[i,"value3"]))
}
...
save.image(file="getsimexp3option_ran.RData")
