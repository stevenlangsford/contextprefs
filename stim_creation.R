...
## ppnt_payoffprior_mean = rep(8,hm_ppnts)
...
if(!exists("ppnt_payoffprior_mean"))stop("Sourced stim_creation without payoff prior mean set")
if(!exists("ppnt_payoffprior_sd"))stop("Sourced stim_creation without payoff prior sd set")
...
appendChoiceReturn <- function(exp.df){ #assumes choice is coded as 1=A,2=B,3=C
...
assignStim <- function(stim,ppntid){
    ...
    stim$payoffprior_mean = ppnt_payoffprior_mean[ppntid]
    stim$payoffprior_sd = ppnt_payoffprior_sd[ppntid]
    return(stim);
}
...
getCustomTrial <- function(probA=.5,probB=.5,probC=.5,payoffA=100,payoffB=100,payoffC=100){
    return(
        data.frame(
            probA = probA,
            probB = probB,
